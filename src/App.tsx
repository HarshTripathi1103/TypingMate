...
const App: React.FC = () => {
    ...
    const GROQ_API_KEY = "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";
    ...
    const generateRandomQuote = async () => {
        try {
            ...
            const prompt = ChatPromptTemplate.fromMessages([
                ["system", "You are a quote generator. Generate a unique, inspirational quote on different topics."],
                ["human", "Create a random quote that is exactly one sentence long."]
            ]);
            ...
            const quote = response.content.toString().trim();
        ...
        return target.split('').map((char, index) => {
            ...
            return (
                <span
                    key={index}
                    className={`relative ${
                        inputChar === undefined ? 'text-black' :
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
            <Card
                ...
                {complete && (
                    <CardFooter className="bg-green-100 border border-green-300 rounded-md p-4 text-center
                     flex
                     flex-col
                    "
                        aria-label="Test results"
                    >
                        <p className="text-gray-700">Time taken to complete: {elapsed?.toFixed(2)} sec</p>
                        <p className="text-blue-700">Words Per Minute (WPM): {wpm}</p>
                        <p className="text-red-700">Errors: {errors}</p>
                        <p className="text-green-700">Accuracy: {((1 - (errors / target.length)) * 100).toFixed(2)}%</p>
                    </CardFooter>
                )}
            </Card>
        </div>
